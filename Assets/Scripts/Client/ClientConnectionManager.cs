...
namespace TMG.NFE_Tutorial
{
    public class ClientConnectionManager : MonoBehaviour
    {
        ...
        {
            var serverWorld = ClientServerBootstrap.CreateServerWorld("Test Server World");

            var serverEndpoint = NetworkEndpoint.AnyIpv4.WithPort(Port);
            {
                using var networkDriverQuarry = serverWorld.EntityManager.CreateEntityQuery(ComponentType.ReadWrite<NetworkStreamDriver>());
                networkDriverQuarry.GetSingletonRW<NetworkStreamDriver>().ValueRW.Listen(serverEndpoint);
            }
        }

        private void StartClient()
        {
            var clientWorld = ClientServerBootstrap.CreateClientWorld("Test Client World");
            var clientEndpoint = NetworkEndpoint.Parse(Address, Port);
            {
                using var networkDriverQuarry = clientWorld.EntityManager.CreateEntityQuery(ComponentType.ReadWrite<NetworkStreamDriver>());
                networkDriverQuarry.GetSingletonRW<NetworkStreamDriver>().ValueRW.Connect(clientWorld.EntityManager, clientEndpoint);
            }
            ...
            var teamRequest = clientWorld.EntityManager.CreateEntity();
            clientWorld.EntityManager.AddComponentData(teamRequest, new ClientTeamRequest
            {
                Value = team
            });
